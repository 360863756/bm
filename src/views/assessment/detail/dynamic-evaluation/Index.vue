...
export default {
  ...
  created() {
    this.assessment = parseJson(this.normalizedMenus, 'path', '/assessment/detail/dynamic-evaluation')[0].children;
    this.getData();
  },
  watch: {
    $route(to, from) {
  ...
  methods: {
    ...
    fetchCardInfo(cb) {
      ...
      this.$api.dynamicEvaluation.queryDetailData(data).then((res) => {
        let data = res.data;
        let info = data.fxpg_pgzx_dtpg_his_data[0];
        if (info) {
