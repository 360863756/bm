          ...
          <div class="item-content">
            <div  class="item-content-header">
              ...
              <div v-else>
                ...
                <br>
                <el-button v-if="!(controlInfo.sfzx && controlInfo.sfzx === '1')" @click="handleExec" class="footer_btn save excute" >确认执行</el-button>
              </div>
            </div>
          </div>
        </el-timeline-item>
  ...
  methods: {
    ...
    fetchSuggestions(queryString, cb) {
      ...
      this.$api.staticEvaluation.queryPoliceList().then((res) => {
        ...
        console.log(_this.policeList);
        cb(_this.filterPolice(queryString, _this.policeList));
      });
    },
    filterPolice(str, list) {
      return list.filter(d => {
        return (d.value.indexOf(str) === 0);
      });
    },
    handleSave(e) {
      let data = { ...this.controlInfo };
      delete data.id;
      ...
      if (!this.controlInfo.sffh || this.controlInfo.sffh === '0') {
        data.sffh = '1';
      }
      this.$api.staticEvaluation.updateControlInfo(data).then((res) => {
        if (data.sffh) {
    ...
    fetchControlData() {
      this.$api.assessment.queryControlData({ ...this.$route.query, pglx: '静态' }).then(res => {
        if (res.errcode === 0) {
